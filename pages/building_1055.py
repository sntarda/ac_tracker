...
# Function to display the Building 1055 page
def building_1055_page():
    st.title("Building 1055")

    units = load_units('data/units.csv')
    building_units = units[units['Facility'] == '1055 S Sherman']

    if building_units.empty:
        st.write("No units found for Building 1055.")
    else:
